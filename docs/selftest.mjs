...
const api = new Function('document',
  `${js}; return { kind, kindLabel, when, list, card, swiftOf, render, hero, dlURL, DATA: v => DATA = v };`
)(document);
...
// download.developer.apple.com URLs go through Apple's services-account downloader
assert.equal(
  api.dlURL('https://download.developer.apple.com/Developer_Tools/Xcode_27_beta_6/Xcode_27_beta_6.xip'),
  'https://developer.apple.com/services-account/download?path=/Developer_Tools/Xcode_27_beta_6/Xcode_27_beta_6.xip');
assert.equal(api.dlURL('https://elsewhere.example/x.xip'), 'https://elsewhere.example/x.xip', 'other hosts untouched');
assert.ok(h.includes('class="btn primary" href="https://dl.example/x.xip"'), 'Download button is the package link');
